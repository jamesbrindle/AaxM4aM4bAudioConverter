    ...
    public partial class AaxAudioConverter : IPreviewTitle, IDisposable
    {
        ...
        private IEnumerable<AaxFileItem> addFilesParallel(IEnumerable<string> filenames)
        {
            ...
            try
            {
                Parallel.ForEach(filenames, DefaultParallelOptions, path =>
                {
                    ...
                    succ = ext == AAX || ext == AA || ext == M4A || ext == M4B;
                    if (!succ)
                        return;
        ...
        private bool checkActivationAndGetChapters(Book.Part part)
        {
            lock (_lockable)
            {
                while (true)
                {
                    ...
                    if (Path.GetExtension(part.ApplicableInFileNameForTranscode).ToLower() == ".m4a" ||
                        Path.GetExtension(part.ApplicableInFileNameForTranscode).ToLower() == ".m4b")
                    {
                        succ = verifyActivationAndGetChapters(part, null);
                        if (!succ.HasValue)
                            return false;
                        if (succ.Value)
                            return true;
        ...
        private bool? verifyActivationAndGetChapters(Book.Part part, string activationcode)
        {
            ...
            if (succ)
            {
                if (!(ffmpeg.IsAaFile ||
                    ffmpeg.IsAaxFile ||
                    Path.GetExtension(part.ApplicableInFileNameForTranscode).ToLower() == ".m4a" ||
                    Path.GetExtension(part.ApplicableInFileNameForTranscode).ToLower() == ".m4b"))
                {
