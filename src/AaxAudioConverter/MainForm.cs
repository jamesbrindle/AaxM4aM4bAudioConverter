...
namespace audiamus.aaxconv
{
    ...
    public partial class MainForm : Form
    {
        ...
        private async void btnAddFile_Click(object sender, EventArgs e)
        {
            ...
            radBtnMp4.Enabled = true;
            foreach (var filename in filenames)
            {
                if (Path.GetExtension(filename).ToLower() == ".m4a" ||
                    Path.GetExtension(filename).ToLower() == ".m4b")
                {
                    radBtnMp3.Checked = true;
                    radBtnMp4.Enabled = false;
        ...
        private void btnRem_Click(object sender, EventArgs e)
        {
            ...
            radBtnMp4.Enabled = true;
            foreach (var filename in _fileItems.Select(m => m.FileItem.FileName))
            {
                if (Path.GetExtension(filename).ToLower() == ".m4a" ||
                    Path.GetExtension(filename).ToLower() == ".m4b")
                {
